...
def get_train_data(batch_size=60,time_step=1,train_begin=0,train_end=5800):
    batch_index=[]
...
def get_test_data(time_step=1,test_begin=5800):
...
def train_lstm(batch_size=1,time_step=1,train_begin=1,train_end=5800):
...
def prediction(time_step=1,test_begin=5800):
    X=tf.placeholder(tf.float32, shape=[None,time_step,input_size])
    #Y=tf.placeholder(tf.float32, shape=[None,time_step,output_size])
