...
def get_train_data(train_begin=TB,train_end=TE):
    ...
    return normalized_train_data,mean,std	

data,mean,std=get_train_data()
#data=get_train_data()
...
if __name__ == '__main__':
    ...
    #print("-------")
    #print(seq[0])
    #print(seq[0,:,np.newaxis])	
    for step in range(len(seq)):
        _, cost, state, pred = sess.run(
                [model.train_op, model.cost, model.cell_final_state, model.pred],
                feed_dict={model.xs:[seq[step,:]], model.ys:[res[step,:]]})
        predict=pred.reshape((-1))
        test_predict.extend(predict)
    test_predict=np.array(test_predict)*std[3]+mean[3]
    res=np.array(res)*std[7]+mean[7]
    np.savetxt('Output_test_predict.csv', test_predict, delimiter=",")
    np.savetxt('Output_test_Y.csv', res, delimiter=",")
    plt.figure()
    plt.plot(list(range(len(test_predict))), test_predict, color='b')
    #print(res.reshape(-1))
    plt.plot(list(range(len(res))), res.reshape(-1), color='r')
    plt.show()